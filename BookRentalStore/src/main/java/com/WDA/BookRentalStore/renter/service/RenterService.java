...
public class RenterService {
    ...
    public Renter getById(Integer id) {
        return repository.findById(id)
                .orElseThrow(() -> new RenterNotFoundException("Renter with ID: " + id + " not found"));

    }

    public Renter save(RenterDto dto) {
        if (repository.findByRenterEmail(dto.renterEmail()).isPresent()) {
            throw new RenterAlreadyExistsException("error.conflict_email");
        }

        if (repository.findByRenterCpf(dto.renterCpf()).isPresent()) {
            throw new RenterAlreadyExistsException("error.conflict_cpf");
        }
    ...
    public void delete(Integer id) {
        ...
        // Injetar RentalRepository seria melhor, mas para manter simplicidade/escopo:
        if (rentalRepository.existsByRenter(renter)) {
