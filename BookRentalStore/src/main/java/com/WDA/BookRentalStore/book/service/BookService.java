...
public class BookService {
    ...
    public Book save(BookDto dto){
        if (repository.findByBookTitle(dto.bookTitle()).isPresent()) {
            throw new BookAlreadyExistsException("Livro com título: " + dto.bookTitle() + " já cadastrado.");
        }
        Book book = new Book();
    ...
    public Book update(Integer id, BookDto dto){
        ...
        if (bookWithSameTitle.isPresent() && !bookWithSameTitle.get().getId().equals(id)){
            throw new BookAlreadyExistsException("O título '"+ dto.bookTitle() +"' já está sendo usado por outro livro");
        }
